 ...
 *
 * FIX: previously returned an array of bare id strings, but callers
 * (app.js's runPathFind) treat each path entry as a full deity
 * object — reading .id, .pantheon, .traits, and spreading it into a
 * graph node. Now builds the BFS over ids internally (fast lookups)
 * but maps the final path back to full deity objects before
 * returning, matching the non-worker findPath in utils/similarity.js.
 */
function findPath(startId, endId, deities, method = 'cosine', threshold = 0.3) {
  ...
  while (queue.length > 0) {
    ...
    for (const neighbor of (adjacency.get(current) || [])) {
      ...
      if (neighbor === endId) {
        // Map id path -> full deity objects before returning
        return newPath.map(id => byId.get(id)).filter(Boolean);
      }
