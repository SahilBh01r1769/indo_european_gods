import { DEITIES, TRAITS, PANTHEON_COLORS, getTraitValue } from '../data/deities.js';
import { COGNATE_PAIRS, getCognate } from '../data/cognates.js';
import { computeSimilarity, sharedTraits } from '../utils/similarity.js';
import { ARCHETYPES, RELATION_KIND_OVERRIDES, RELATION_META, STORIES } from './config.js';
...
function inferKind(cognate) {
  ...
  if (/direct (etymological )?cognate|name cognate|reflex|from pie|linguistic/.test(note)) return 'linguistic';
  if (/roman interpretatio|hellenistic|identified|derived from|fusion|transmission/.test(note)) return 'historical';
  if (cognate.confidence === 'proposed') return 'speculative';
  if (/archetype|structural|serpent|parallel|both/.test(note)) return 'structural';
  return 'comparative';
}
...
function bestSharedTraits(a, b) {
  return sharedTraits(a, b, 0.38)
    .map(name => ({ name, weight: Math.min(getTraitValue(a, name), getTraitValue(b, name)) }))
    .sort((x, y) => y.weight - x.weight);
}

function clueLabel(kind, shared, cognate) {
  if (kind === 'linguistic') return 'A name survives';
  if (kind === 'historical') return 'A meeting of traditions';
  if (kind === 'speculative') return shared[0] ? (TRAIT_CLUES[shared[0].name] || shared[0].name) : 'A strange echo';
  if (cognate?.note?.toLowerCase().includes('serpent')) return 'The serpent adversary';
...
export function relationBetween(aId, bId) {
  ...
  return {
    ...
    clue: clueLabel(kind, shared, cognate),
...
function relationPriority(r) {
  const order = { linguistic: 0, historical: 1, structural: 2, comparative: 3, speculative: 4, model: 5 };
...
export function candidateConnections(id, discoveredIds = [], max = 4) {
  ...
  const modelCandidates = DEITIES
    .filter(d => d.id !== deity.id && !discovered.has(d.id) && d.pantheon !== deity.pantheon)
  ...
  for (const relation of modelCandidates) {
    if (!candidates.has(relation.target)) candidates.set(relation.target, relation);
  }
  ...
  return [...candidates.values()]
    ...
    .map((relation, index) => ({
      ...
      label: relation.clue,
      hint: `${getDeity(relation.target)?.pantheon || 'Another'} tradition`,
      relation,
      index,
    }));
}
...
export function archetypeMembers(archetypeOrId, limit = 10) {
  const archetype = typeof archetypeOrId === 'string' ? archetypeById(archetypeOrId) : archetypeOrId;
  if (!archetype) return [];
  return DEITIES
    .map(deity => {
      const parts = archetype.traits.map(trait => getTraitValue(deity, trait));
      const average = parts.reduce((sum, x) => sum + x, 0) / Math.max(1, parts.length);
      const coverage = parts.filter(x => x >= 0.45).length / Math.max(1, parts.length);
      const seeded = archetype.seeds.includes(deity.id) || archetype.suggested.includes(deity.id);
      const score = average * 0.72 + coverage * 0.18 + (seeded ? 0.10 : 0);
      return { deity, score, breakdown: archetype.traits.map((trait, i) => ({ trait, value: parts[i] })) };
    })
...
export function emergentArchetypes(discoveredIds) {
  const discovered = new Set(discoveredIds);
  return ARCHETYPES
    .map(archetype => ({ archetype, members: archetypeMembers(archetype, 20).filter(x => discovered.has(x.deity.id)) }))
...
export function deityGlyph(deity) {
  if (!deity) return '·';
  const script = `${deity.originalScript || ''}`.trim();
  if (script && script.length <= 6) return script;
...
export function searchMythos(query, limit = 12) {
  ...
  const deityHits = DEITIES.map(deity => {
    const hay = [deity.id, deity.pantheon, deity.epithet, deity.desc, ...(deity.domains || []), ...(deity.symbols || []), ...Object.keys(deity.traits || {})].join(' ').toLowerCase();
    let score = 0;
  ...
  const archetypeHits = ARCHETYPES.map(archetype => {
    const hay = [archetype.name, archetype.short, archetype.description, ...archetype.traits].join(' ').toLowerCase();
    return { type: 'archetype', archetype, score: hay.includes(q) ? 18 : 0 };
  }).filter(x => x.score > 0);

  const storyHits = STORIES.map(story => {
    const hay = [story.title, story.deck, ...story.path].join(' ').toLowerCase();
    return { type: 'story', story, score: hay.includes(q) ? 12 : 0 };
  }).filter(x => x.score > 0);

  return [...deityHits, ...archetypeHits, ...storyHits].sort((a, b) => b.score - a.score).slice(0, limit);
}
...
export function compareDeities(ids) {
  ...
  for (let i = 0; i < deities.length; i++) {
    for (let j = i + 1; j < deities.length; j++) pairs.push(relationBetween(deities[i], deities[j]));
  }
  const traitRows = TRAITS.map(trait => ({ trait, values: deities.map(d => getTraitValue(d, trait)) }))
