...
function jitter(id, amount = 24) {
  ...
  return ((Math.abs(hash) % 1000) / 999 - .5) * amount;
}
...
export class MythGraph {
  ...
  render(state) {
    ...
    const edges = state.discoveredEdges
      .map(edge => ({ ...edge, sourceDeity: getDeity(edge.source), targetDeity: getDeity(edge.target) }))
      .filter(edge => edge.sourceDeity && edge.targetDeity);

    if (state.mode === 'time') return this.renderTime(discovered, edges, state, width, height);
    if (state.mode === 'geography') return this.renderGeography(discovered, edges, state, width, height);
  ...
  renderNetwork(discovered, edges, state, width, height) {
    ...
    nodesMerged.select('.node-halo')
      ...
      .style('stroke', node => node.type === 'deity' ? deityAccent(node.deity) : '#b99d68');

    nodesMerged.select('.node-disc')
      .attr('r', node => node.type === 'deity' ? 24 : 21)
      .style('stroke', node => node.type === 'deity' ? deityAccent(node.deity) : '#9d8d78');

    nodesMerged.select('.node-glyph')
      .style('fill', node => node.type === 'deity' ? deityAccent(node.deity) : '#6b6258')
    ...
    this.simulation = d3.forceSimulation(nodes)
      .force('link', d3.forceLink(links)
        ...
        .strength(link => link.type === 'clue' ? .72 : .42))
      .force('charge', d3.forceManyBody().strength(node => node.type === 'clue' ? -500 : -720))
      .force('center', d3.forceCenter(width * .52, height * .48))
      .force('collision', d3.forceCollide().radius(node => node.type === 'clue' ? 78 : 88))
      .alpha(.85)
      .alphaDecay(.04)
      .on('tick', () => {
        nodes.forEach(node => {
    ...
    nodesMerged.call(d3.drag()
      .on('start', (event, node) => {
        if (!event.active) this.simulation.alphaTarget(.18).restart();
        node.fx = node.x;
        node.fy = node.y;
      })
      .on('drag', (event, node) => {
  ...
  renderStatic(discovered, edges, state, positionFor, { dimFuture = false } = {}) {
    this.simulation?.stop();
    const nodes = discovered.map(deity => ({ id: deity.id, deity, ...positionFor(deity) }));
    ...
    const edgeSelection = this.edgeLayer.selectAll('line.graph-edge').data(links, edge => edge.id);
    edgeSelection.exit().remove();
    ...
    const nodeSelection = this.nodeLayer.selectAll('g.graph-node').data(nodes, node => node.id);
    ...
    merged.select('.node-halo')
      .attr('r', node => node.id === state.selectedNode ? 34 : 29)
      .style('stroke', node => deityAccent(node.deity));
    merged.select('.node-disc').attr('r', 23).style('stroke', node => deityAccent(node.deity));
    merged.select('.node-glyph').style('fill', node => deityAccent(node.deity)).text(node => deityGlyph(node.deity));
    merged.select('.node-name').attr('y', 48).text(node => node.deity.id);
    merged.select('.node-meta').attr('y', 63).text(node => node.deity.pantheon);
  }
  ...
  renderTime(discovered, edges, state, width, height) {
    ...
    const maxEra = 1400;
    const x = d3.scaleLinear().domain([minEra, maxEra]).range([margin.left, width - margin.right]);
    const pantheons = [...new Set(discovered.map(deity => deity.pantheon))];
    const y = d3.scalePoint().domain(pantheons).range([margin.top + 32, height - margin.bottom - 20]).padding(.45);
  ...
  renderGeography(discovered, edges, state, width, height) {
    ...
    [...new Set(discovered.map(deity => deity.pantheon))].forEach(pantheon => {
  ...
  fit() {
    this.svg.transition().duration(260).call(this.zoom.transform, d3.zoomIdentity);
  }
}
