...
import { candidateConnections, relationBetween, archetypeMembers, compareDeities } from '../src/v3/model.js';
import { startWithDeity, revealClue, getState, resetJourney, beginStory, revealStoryNext } from '../src/v3/state.js';
...
test('revealing a clue grows the persistent journey rather than replacing it', () => {
  ...
  startWithDeity('Thor');
  const clue = candidateConnections('Thor', ['Thor'], 4).find(c => c.target === 'Indra');
  ...
  assert.ok(state.discoveredEdges.some(e => e.id.includes('Indra') && e.id.includes('Thor')));
});
...
test('comparison keeps qualitative evidence plus model overlap data', () => {
  ...
  assert.ok(result.pairs.every(p => typeof p.fit === 'string' && typeof p.score === 'number'));
});
